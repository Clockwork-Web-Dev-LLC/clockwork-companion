...
### Fixed
- **Sidebar menu icon**: the bundled logo mark's two embedded raster layers (the colored circle backdrop and accent spike) declared an invalid data-URI MIME type (`img/png` instead of `image/png`), which browsers silently refused to decode — only the bare white "C" glyph rendered in the admin sidebar. Corrected both occurrences in `assets/clockwork-logo-mark.svg` and bumped the asset's cache-busting query param so browsers/CDNs that cached the broken file refetch it.

### Changed
- **Activity page icons**: `cache_purged`, `comments_cleanup`, `wfls_plugin_removed`, and `site.provider_migrated` — action types mirrored in from Clockwork's own ops tool via `ActionLogAppendRoute` rather than generated by this plugin — now get a proper icon and label in the monthly summary tiles and the activity table, instead of falling through to the generic circle and an auto-titlecased label.
